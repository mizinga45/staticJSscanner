# scanner/rules/open_redirect.py
...
REDIRECT_FUNCTIONS = {'redirect', 'replace', 'assign'}


class OpenRedirectRule(VulnerabilityRule):
    def __init__(self):
        super().__init__(
            vuln_type="Open Redirect",
            cwe_id="CWE-601",
            description="User input used in URL redirect without validation."
        )
    ...
    def _walk(self, node, file_path, code_lines, vulns, tainted):
        ...
        if node_type == 'VariableDeclaration':
            ...
            # window.location = tainted / window.location.href = tainted
            if left and left.get('type') == 'MemberExpression':
                prop = left.get('property', {}).get('name', '')
                obj_name = self._get_full_name(left.get('object'))
                if prop in ('href', 'location') or obj_name in ('window.location', 'location', 'document.location'):
        ...
        # res.redirect(tainted) or location.replace(tainted)
        if node_type == 'CallExpression':
            callee = node.get('callee')
            func_name = ''
            if callee and callee.get('type') == 'MemberExpression':
                func_name = callee.get('property', {}).get('name', '')
            elif callee and callee.get('type') == 'Identifier':
                func_name = callee.get('name', '')

            if func_name in REDIRECT_FUNCTIONS:
